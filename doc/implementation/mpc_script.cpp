...
ACADO::DMatrix optimize_path(
  ...
  DifferentialState elevator, aileron;
  DifferentialState rudder, throttle;
  ...
  C_X0   = 0;
  C_Xalp = - C_D*cos(alpha_trim)
           + C_L*sin(alpha_trim);
  C_Xde  = - C_Dde*cos(alpha_trim)
           + C_Lde*sin(alpha_trim);
  C_Xq   = - C_Dq*cos(alpha_trim)
           + C_Lq*sin(alpha_trim);

  C_Z0 = 0;
  C_Zalp = - C_D*sin(alpha_trim)
           - C_L*cos(alpha_trim);
  C_Zde  = - C_Dde*sin(alpha_trim)
           - C_Lde*cos(alpha_trim);
  C_Zq   = - C_Dq*sin(alpha_trim)
           - C_Lq*cos(alpha_trim);
  ...
  Yv = ((rho*S*b*v_trim)/(4*mass*Va_trim))
           * (C_Yp*p_trim + C_Yr*r_trim)
     + ((rho*S*v_trim)/mass)
           * (C_Y0 + C_Ybeta*beta_trim
           + C_Yda*aileron_trim + C_Ydr*rudder_trim)
     + ((rho*S*C_Ybeta)/(2*mass))
           * sqrt(u_trim*u_trim + w_trim*w_trim);
  ...
  Lv = ((rho*S*b*b*v_trim)/(4*Va_trim))
           * (C_pp*p_trim + C_pr*r_trim)
  ...
  Nv = ((rho*S*b*b*v_trim)/(4*Va_trim))
           *(C_rp*p_trim + C_rr*r_trim)
  ...
  Xu = ((u_trim*rho*S)/mass)*(C_X0 + C_Xalp*alpha_trim
                                + C_Xde*elevator_trim)
     - ((rho*S*w_trim*C_Xalp)/(2*mass))
     + ((rho*S*c*C_Xq*u_trim*q_trim)/(4*mass*Va_trim))
     - ((rho*S_prop*C_prop*u_trim)/mass);

  Xw = -q_trim+((w_trim*rho*S)/mass)
           *(C_X0 + C_Xalp*alpha_trim
                  + C_Xde*elevator_trim)
     + ((rho*S*c*C_Xq*w_trim*q_trim)/(4*mass*Va_trim))
     + ((rho*S*C_Xalp*u_trim)/(2*mass))
                  - ((rho*S_prop*C_prop*w_trim)/mass);

  Xq  = -w_trim + ((rho*Va_trim*S*C_Xq*c)/(4*mass));
  Xde = ((rho*Va_trim*Va_trim*S*C_Xde)/(2*mass));
  Xdt = ((rho*S_prop*C_prop*k_motor
                    *k_motor*throttle_trim)/mass);
    
  Zu = q_trim + ((u_trim*rho*S)/mass)*(C_Z0
           + C_Zalp*alpha_trim + C_Zde*elevator_trim)
     - ((rho*S*C_Zalp*w_trim)/(2*mass))
     + ((u_trim*rho*S*C_Zq*c*q_trim)
                    /(4*mass*Va_trim));
    
  Zw = ((w_trim*rho*S)/mass)*(C_Z0
           + C_Zalp*alpha_trim + C_Zde*elevator_trim)
     + ((rho*S*C_Zalp*u_trim)/(2*mass))
     + ((rho*w_trim*S*c*C_Zq*q_trim)
                    /(4*mass*Va_trim));
  ...
  Mu = ((u_trim*rho*S*c)/J_y)*(C_m0 
           + C_malp*alpha_trim + C_mde*elevator_trim)
     -((rho*S*c*C_malp*w_trim)/(2*J_y))
     +((rho*S*c*c*C_mq*q_trim*u_trim)
                    /(4*J_y*Va_trim));

  Mw = ((w_trim*rho*S*c)/J_y)*(C_m0
             + C_malp*alpha_trim
             + C_mde*elevator_trim)
     +((rho*S*c*C_malp*u_trim)/(2*J_y))
     +((rho*S*c*c*C_mq*q_trim*w_trim)
                    /(4*J_y*Va_trim));
  ...
  p_N_dot = cos(theta+theta_trim)
            * cos(psi+psi_trim)*(u+u_trim)
            + (sin(phi+phi_trim)
            * sin(theta+theta_trim)*cos(psi+psi_trim)
            - cos(phi+phi_trim)*sin(psi+psi_trim))
            * (v+v_trim)
            + (cos(phi+phi_trim)
            * sin(theta+theta_trim)*cos(psi+psi_trim)
            + sin(phi+phi_trim)
            * sin(psi+psi_trim))*(w+w_trim);
	
  p_E_dot = cos(theta+theta_trim)
            * sin(psi+psi_trim)*(u+u_trim)
            + (sin(phi+phi_trim)
            * sin(theta+theta_trim)*sin(psi+psi_trim)
            + cos(phi+phi_trim)
            * cos(psi+psi_trim))*(v+v_trim)
            + (cos(phi+phi_trim)
            * sin(theta+theta_trim)*sin(psi+psi_trim)
            - sin(phi+phi_trim)
            * cos(psi+psi_trim))*(w+w_trim);

  p_D_dot = -sin(theta)*u + sin(phi)*cos(theta)*v
            + cos(phi)*cos(theta)*w;
  ...
  f << dot(v) == Yv*v + Yp*p + Yr*r
              + g*cos(theta_trim)*cos(phi_trim)*phi
              + Yda*aileron + Ydr*rudder;
    
  f << dot(p) == Lv*v + Lp*p + Lr*r
              + Lda*aileron + Ldr*rudder;

  f << dot(r) == Nv*v + Np*p + Nr*r
              + Nda*aileron + Ndr*rudder;

  f << dot(phi) == p 
                + cos(phi_trim)*tan(theta_trim)*r
                + (q_trim*cos(phi_trim)
                         *tan(theta_trim)
                - r_trim*sin(phi_trim)
                         *tan(theta_trim))*phi;
  ...
  f << dot(u) == Xu*u + Xw*w + Xq*q
              - g*cos(theta_trim)*theta 
              + Xde*elevator + Xdt*throttle;

  f << dot(w) == Zu*u + Zw*w + Zq*q
              - g*sin(theta_trim)*theta
              + Zde*elevator;
  ...
  f << dot(h) == sin(theta_trim)*u
              - cos(theta_trim)*w
              + (u_trim*cos(theta_trim)
              + w_trim*sin(theta_trim))*theta;
  ...
  GAMMA = -atan(p_D_dot/sqrt(p_N_dot*p_N_dot
                           + p_E_dot*p_E_dot
                           + p_D_dot*p_D_dot));
  CHI = asin(p_E_dot/sqrt(p_N_dot*p_N_dot
                        + p_E_dot*p_E_dot
                        + p_D_dot*p_D_dot));
  ...
  cx = p_N + (h+h_trim)*tan(theta+theta_trim)
                       *cos(psi+psi_trim)
           - (h+h_trim)*tan(-phi+phi_trim)
                       *sin(psi+psi_trim);
  cy = p_E + (h+h_trim)*tan(theta+theta_trim)
                       *sin(psi+psi_trim)
           + (h+h_trim)*tan(-phi+phi_trim)
                       *cos(psi+psi_trim);
